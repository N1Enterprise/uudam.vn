                                ...
                                <div class="form-group">
                                    ...
                                    <select name="product_id" title="--{{ __('Select Product') }}--" class="form-control k_selectpicker" data-size="5" data-live-search="true" {{ $productReview->is_real_user ? 'disabled' : '' }}>
                                        ...
                                        <optgroup label="{{ $category->name }}">
                                            ...
                                            <option value="{{ $product->id }}" {{ old('product_id', $productReview->product_id) == $product->id ? 'selected' : '' }} data-product-type="{{ $product->type }}">{{ $product->name }} ({{ $product->type_name }})</option>
                                            @endforeach
                                        </optgroup>
                                        @endforeach
                                    </select>
