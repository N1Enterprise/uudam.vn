                    ...
                    <div class="form-group">
                        ...
                        <select name="product_id" title="-- {{ __('Chọn sản phẩm') }} --" class="form-control k_selectpicker" data-size="5" data-live-search="true">
                            ...
                            <optgroup label="{{ $category->name }}">
                                ...
                                <option value="{{ $product->id }}" data-product-type="{{ $product->type }}" data-tokens="{{ $product->id }} | {{ $product->name }} | {{ $product->code }}" data-categories='@json($product->categories->pluck('id')->toArray())'>{{ $product->name }} ({{ $product->type_name }})</option>
                                @endforeach
                            </optgroup>
                            @endforeach
                        </select>
    ...
    function renderCallbackImage(data, type, full) {
        const wrapper = $(`
            <div style="width: 200px;">
                ...
                <a href="${full.product.actions.update}" target="_blank" title="(${full.product.id}) ${full.product.name}" class="d-block mt-2">Xem sản phẩm</a>
            </div>
        `);
