	...
	<div class="row">
		<div class="col-md-6">
			...
			<div class="k-portlet k-portlet--tabs">
				...
				<form class="k-form" id="update_payment_option" method="post" action="{{ route('bo.web.payment-options.update', $paymentOption->getKey()) }}">
					@csrf
					@method('PUT')
						...
						<div class="tab-content">
							<div class="tab-pane active show" id="mainTab" role="tabpanel">
								...
								<div class="form-group">
									...
									<select class="form-control k_selectpicker" id="type" name="type" required>
										@foreach ($paymentOptionTypes as $type => $label)
											...
											<option data-append="{{ $appendElementId }}"
												{{ old('type', $paymentOption->type) == $type ? 'selected' : '' }}
												value="{{ $type }}"
												> {{ $label }}
											</option>
										@endforeach
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="min_amount" placeholder="{{ __('Enter min amount') }}" value="{{ old('min_amount', $paymentOption->min_amount) }}">
								</div>
								<div class="form-group">
									<label>{{ __('Max Amount') }}</label>
									<input type="text" class="form-control" name="max_amount" placeholder="{{ __('Enter max amount') }}" value="{{ old('max_amount', $paymentOption->max_amount) }}">
								</div>
									...
									<div class="col-3 col-sm-3">
										<span class="k-switch">
											<label>
												...
												@else
												<input type="checkbox" {{ $paymentOption->isActive() ? 'checked' : ''}} name="status"/>
												@endif
												<span></span>
											</label>
										</span>
								...
								<div class="row">
									...
									<div class="col-4 col-sm-3">
										<span class="k-switch">
											<label>
												<input type="hidden" name="display_on_frontend" value="0">
												<input type="checkbox" {{ old('display_on_frontend', $paymentOption->display_on_frontend) == '1'  ? 'checked' : ''}} value="1" name="display_on_frontend"/>
												<span></span>
											</label>
										</span>
									</div>
							...
							<div class="tab-pane" id="cmsTab" role="tabpanel">
                                <div id="restrictionTab">
                                    <div class="form-group">
                                        ...
                                        <select data-actions-box="true" name="payment_option_allowed_countries[]" title="--{{ __('Select Country') }}--" data-size="5" data-live-search="true" class="form-control k_selectpicker payment_option_allowed_country_selector" multiple data-selected-text-format="count > 5">
                                            @foreach($countries as $country)
                                                <option
                                                    {{ in_array($country->iso2, old("payment_option_allowed_countries", $paymentOption->allowedCountries->pluck('iso2')->all())) ? 'selected' : '' }}
				...
				<input type="hidden" id="oldPaymentProvider" value="{{ old('payment_provider_id', $paymentOption->payment_provider_id) }}">
				<input type="hidden" id="oldDepositBank" value="{{ old('deposit_bank_id', $paymentOption->deposit_bank_id) }}">
				<input type="hidden" id="oldOnlineBankCode" value="{{ old('online_banking_code', $paymentOption->online_banking_code) }}">
				<input type="hidden" id="oldBlockchainNetwork" value='@json(old("blockchain_networks", $paymentOption->blockchain_networks)))'>
				...
				<input type="hidden" id="paymentOptionType" value="{{ $paymentOption->type }}">
...
<script>
    ...
    $('select#payment_provider_id').on('change', function() {
        ...
        let paymentOptionTypeId = $('select[name=type]').val();
        let paymentMethodCode = params.payment_method ? params.payment_method[paymentOptionTypeId] : '';
        ...
        for (bankCode in paymentChannels) {
            ...
            for (let option in paymentChannels[bankCode]) {
                let selected = $('input#oldOnlineBankCode').val() == option;
                let bankOptionElement = $('<option>').attr('selected', onlineBankSelected == option).attr('value', option).text(paymentChannels[bankCode][option]).prop('selected', selected);
    ...
    function onChangeCurrency() {
        ...
        if (currencyCodeSelected.attr('data-type') == '{{ $currencyTypeEnum::CRYPTO }}') {
            ...
            $('.online_banking_code_wrapper').addClass('d-none');
            let optionSelected = $('input#oldPaymentOptionType').val() ?? $('input#paymentOptionType').val();
            ...
            $('.online_banking_code_wrapper').removeClass('d-none');
            let optionSelected = $('input#oldPaymentOptionType').val() != '' ? $('input#oldPaymentOptionType').val() : $('input#paymentOptionType').val();
...
<script>
    ...
    $(document).ready(function () {
        $('form#update_payment_option').find('input[name="name"]').on('change keyup', function() {
            let nameVal = $(this).val();

            let nameValSlugify = nameVal?.trim()?.toLowerCase()?.replace(/[^\w-]+/g, '-');

            $('form#update_payment_option').find('input[name="slug"]').val(nameValSlugify);
        })

        $('select.payment_option_allowed_country_selector').on('loaded.bs.select', function() {
            $(this).trigger('changed.bs.select');
        })

        $('select.payment_option_allowed_country_selector').on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
            let $this = $(this)
    ...
    $(document).on('click', '.paymentOptionAllowedCountryBadgeSelected', function() {
        ...
        let selector = $(`select.payment_option_allowed_country_selector`);
