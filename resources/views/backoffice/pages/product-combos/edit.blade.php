			...
			<div class="k-portlet k-portlet--tabs">
				<div class="k-portlet__head">
					<div class="k-portlet__head-label">
						<h3 class="k-portlet__head-title">{{ __('Edit Product Combo') }}</h3>
					</div>
				...
				<!--begin::Form-->
				<form class="k-form" name="form_included_products" id="form_included_products" method="post" action="{{ route('bo.web.product-combos.update', $productCombo->id) }}" enctype="multipart/form-data">
					@csrf
                    @method('PUT')
						...
						<div class="tab-content">
							<div class="tab-pane active show" id="mainTab" role="tabpanel">
								<div class="form-group">
									<label>{{ __('Tên') }} *</label>
									<input type="text" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name" placeholder="{{ __('Nhập tên') }}" value="{{ old('name', $productCombo->name) }}" required>
                                    @error('name')
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="upload_image_custom position-relative">
                                                <input type="text" data-image-ref-path="primary" data-image-ref-index="0" class="form-control image_primary_image_url" name="image[path]" value="{{ old('image.path', $productCombo->image) }}" placeholder="{{ __('Tải ảnh lên hoặc nhập URL ảnh') }}" style="padding-right: 104px;">
                                ...
                                <div class="form-group">
                                    ...
                                    <x-number-input
                                        ...
                                        value='{{ old("sale_price", $productCombo->sale_price) }}'
                                    />
                                    @error("sale_price")
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <input
                                        type="number"
                                        name="stock_quantity"
                                        value="{{ old('stock_quantity', $productCombo->stock_quantity) }}"
                                ...
                                <div class="form-group">
									<label>{{ __('Unit') }} *</label>
									<input type="text" class="form-control {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit" placeholder="{{ __('Enter unit') }}" value="{{ old('unit', $productCombo->unit) }}" required>
                                    @error('unit')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
								</div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="" rows="3" class="form-control">{{ old('description', $productCombo->description) }}</textarea>
                                </div>
								...
								<div class="form-group row">
									...
									<div class="col-3">
										<span class="k-switch">
											<label>
												<input type="checkbox" {{ old('status', boolean($productCombo->status) ? '1' : '0') == '1'  ? 'checked' : ''}} value="1" name="status"/>
												<span></span>
											</label>
										</span>
									</div>
