	...
	<div class="row">
		<div class="col-md-6">
			...
			<div class="k-portlet">
				...
				<form class="k-form" id="update_payment_provider" method="post"
					action="{{ route('bo.web.payment-providers.update', $paymentProvider->getKey()) }}">
					@csrf
					@method('PUT')
					<div class="k-portlet__body">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" name="name" placeholder="{{ __('Enter payment provider name') }}" value="{{ old('name', $paymentProvider->name) }}" required>
						</div>
						...
						<div class="form-group">
							...
							<select class="form-control" id="payment_type" name="payment_type">
								...
								<option {{ old('payment_type', $paymentProvider->payment_type) == $type ? 'selected' : '' }} data-type="{{strtolower($label)}}" value="{{ $type }}">{{ $label }}</option>
								@endforeach
							</select>
						</div>
						@php
						$paymentProviderJsonParams = json_encode($paymentProvider->params, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
						@endphp
						<div class="form-group class_hint_group">
							<div class="k-section__content k-section__content--border">
								<div id="class_hint">
									<div class="card card-body" id="class_hint_content" style="white-space:pre">
						...
						<div class="form-group">
							...
							<input type="hidden" name="params" value="{{ old('params', $paymentProviderJsonParams) }}">
						</div>
						<div class="form-group">
							...
							<div class="k-checkbox-inline">
								...
								<label class="k-checkbox">
									<input type="checkbox" {{ in_array($currency->getKey(), old('supported_currencies', data_get($paymentProvider, 'supported_currencies') ?? [])) ? 'checked' : '' }} value="{{ $currency->getKey() }}" name="supported_currencies[]"> {{ $currency->getKey() }}
									<span></span>
								</label>
								@endforeach
							</div>
							...
							<div class="col-3">
								<span class="k-switch">
									<label>
										...
										@else
										<input type="checkbox" {{ $paymentProvider->isActive() ? 'checked' : ''}} name="status"/>
										@endif
										<span></span>
									</label>
								</span>
...
<script>
	let editor = ace.edit($('.json_editor')[0], {
            ...
            maxLines: Infinity,
            minLines: 15,
			value: $('input[name=params]').val() != '[]' ? $('input[name=params]').val() : '{}'
		});
		...
		$('form#update_payment_provider').on('submit', function(e) {
			e.preventDefault();
