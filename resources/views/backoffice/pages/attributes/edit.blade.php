						...
						<div class="tab-content">
							<div class="tab-pane active show" id="mainTab" role="tabpanel">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="categories[]" title="--{{ __('Select Cagegories') }}--" class="form-control k_selectpicker {{ $errors->has('categories') ? 'is-invalid' : '' }}" data-size="5" multiple data-live-search="true">
                                        @foreach($categoryGroups as $categoryGroup)
                                        <optgroup label="{{ $categoryGroup->name }}">
                                            @foreach($categoryGroup->categories as $category)
                                            <option value="{{ $category->id }}" {{ in_array($category->id, old('categories', $attribute->categories->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>{{ $category->name }}</option>
                                            @endforeach
                                        </optgroup>
                                        @endforeach
                                    </select>
