	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="k-portlet k-portlet--tabs">
				...
				<!--begin::Form-->
				<form class="k-form" name="form-video" id="form-video" method="post" action="{{ route('bo.web.videos.store') }}" enctype="multipart/form-data">
					@csrf
							...
							<div class="tab-pane active show" id="mainTab" role="tabpanel">
								...
								<div class="form-group">
                                    ...
                                    <select name="video_category_id" title="-- {{ __('Chọn Danh mục') }} --" data-toggle="tooltip" data-live-search="true" class="form-control k_selectpicker  {{ $errors->has('video_category_id') ? 'is-invalid' : '' }}">
                                        ...
                                        <option value="{{ $cat->id }}" {{ old('video_category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="upload_image_custom position-relative">
                                                <input type="text" data-image-ref-path="thumbnail" data-image-ref-index="0" class="form-control image_thumbnail_url" name="thumbnail[path]" value="{{ old('thumbnail.path') }}" placeholder="{{ __('Tải ảnh lên hoặc nhập URL ảnh') }}" style="padding-right: 104px;">
                                                <div data-image-ref-wrapper="thumbnail" data-image-ref-index="0" class="d-none w-100 position-absolute d-none" style="top: 50%; left: 4px; transform: translateY(-50%); height: 90%; background-color: #fff;">
                                                    <div class="d-flex align-items-center h-100">
                                                        ...
                                                        <span data-image-ref-delete="thumbnail" data-image-ref-index="0" aria-hidden="true" style="font-size: 16px; cursor: pointer;">&times;</span>
                                                    </div>
                                                </div>
								...
								<div class="form-group">
									...
									<input type="text" name="source_url" class="form-control" placeholder="{{ __('Source URL') }}" value="{{ old('source_url') }}">
								...
								<div class="form-group">
                                    <x-content-editor id="content" label="{{ __('Nội dung') }}" name="content" value="{{ old('content') }}" />
                                </div>
