			...
			<div class="k-portlet">
				<div class="k-portlet__head">
					<div class="k-portlet__head-label">
						<h3 class="k-portlet__head-title">{{ __('Add Withdraw Bank') }}</h3>
					</div>
				...
				<form class="k-form" method="post" action="{{ route('bo.web.withdraw-banks.store') }}" id="store_withdraw_bank">
					@csrf
							...
							<div class="tab-pane active show" id="mainTab" role="tabpanel">
								<div class="form-group">
									...
									<input type="text" class="form-control" name="name"
										placeholder="{{ __('Enter bank name') }}" value="{{ old('name') }}" required>
								</div>
								<div class="form-group">
									<label for="bank_code">{{ __('Bank Code') }} *</label>
									<input type="text" class="form-control" name="code"
									placeholder="{{ __('Enter bank code') }}" value="{{ old('code') }}" required>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<select class="form-control" id="bank_type" name="type" required>
										@foreach ($userBankTypes as $type)
											<option {{ old('type') == data_get($type, 'id') ? 'selected' : '' }} value="{{ data_get($type, 'id') }}">{{ data_get($type, 'name') }}</option>
										@endforeach
									</select>
								</div>
								<div class="form-group d-none blockchain_network">
									<label class="blockchian-network-label">{{ __('Blockchain Networks') }} *</label>
									<select class="form-control k_selectpicker blockchain_network_selector" name="blockchain_networks[]" data-live-search="true" multiple data-selected-text-format="count > 3" required>
									</select>
								</div>
							...
							<div class="tab-pane" id="cmsTab" role="tabpanel">
                                <div id="restrictionTab">
                                    <div class="form-group">
                                        ...
                                        <select data-actions-box="true" name="withdraw_bank_allowed_countries[]" title="--{{ __('Select Country') }}--" data-size="5" data-live-search="true" class="form-control k_selectpicker withdraw_bank_allow_country_selector" multiple data-selected-text-format="count > 5">
                                            @foreach($countries as $country)
                                            <option
                                                {{ in_array($country->iso2, old("withdraw_bank_allowed_countries", [])) ? 'selected' : '' }}
                                    ...
                                    <div class="form-group withdrawBankAllowedCountryOptionHolder mb-0">
                                        <div class="withdrawBankAllowedCountryHolderContent">
                                        </div>
                                    </div>
                                </div>
							</div>
				...
				<input type="hidden" id="oldBankType" value='{{ old('type') }}'>
...
<script>
    ...
    $('#currency_code').on('change', function() {
        ...
        if (currency.data('type') == '{{ $currencyTypeEnum::CRYPTO }}') {
            ...
            $('select[name=blockchain_networks\\[\\]]').attr('required', false);
        }
    });
    ...
    function generateBlockchainNetwork(currencyCode) {
        ...
        let selected = $('input#oldBlockchainNetwork').val();

        blockchainNetworkElement.empty();

        if (blockchainNetWorks) {
            for(let i in blockchainNetWorks) {
                let option = $(`<option value="${data_get(blockchainNetWorks[i], 'network_code')}" ${selected.includes(data_get(blockchainNetWorks[i], 'network_code')) ? 'selected' : ''} >`)
...
<script>
    ...
    $(document).ready(function () {
        $('form#store_withdraw_bank').find('input[name="name"]').on('change keyup', function() {
            let nameVal = $(this).val();

            let nameValSlugify = nameVal?.trim()?.toLowerCase()?.replace(/[^\w-]+/g, '-');

            $('form#store_withdraw_bank').find('input[name="slug"]').val(nameValSlugify);
        })

        $('select.withdraw_bank_allow_country_selector').on('loaded.bs.select', function() {
            $(this).trigger('changed.bs.select');
        })

        $('select.withdraw_bank_allow_country_selector').on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
            let $this = $(this);
    ...
    $(document).on('click', '.withdrawBankAllowedCountryBadgeSelected', function() {
        ...
        let selector = $(`select.withdraw_bank_allow_country_selector`);
