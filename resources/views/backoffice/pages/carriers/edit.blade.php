			...
			<div class="k-portlet k-portlet--tabs">
				<div class="k-portlet__head">
					<div class="k-portlet__head-label">
						<h3 class="k-portlet__head-title">{{ __('Edit Carrier') }}</h3>
					</div>
				...
				<!--begin::Form-->
				<form class="k-form" name="form_carriers" id="form_carriers" method="post" action="{{ route('bo.web.carriers.update', $carrier->id) }}" enctype="multipart/form-data">
					@csrf
                    @method('PUT')
						...
						<div class="tab-content">
							<div class="tab-pane active show" id="mainTab" role="tabpanel">
                                <div class="form-group">
									<label>{{ __('Name') }} *</label>
									<input type="text" class="form-control" name="name" placeholder="{{ __('Enter name') }}" value="{{ old('name', $carrier->name) }}" required>
								</div>

                                <div class="form-group">
									<label>{{ __('Email') }}</label>
									<input type="text" class="form-control" name="email" placeholder="{{ __('Enter email') }}" value="{{ old('email', $carrier->email) }}">
								</div>

                                <div class="form-group">
									<label>{{ __('Phone') }}</label>
									<input type="text" class="form-control" name="phone" placeholder="{{ __('Enter phone') }}" value="{{ old('phone', $carrier->phone) }}">
								</div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="upload_image_custom position-relative">
                                                <input type="text" data-image-ref-path="logo" data-image-ref-index="0" class="form-control image_logo_url" name="logo[path]" value="{{ old('logo.path', $carrier->logo) }}" placeholder="{{ __('Upload Image or Input URL') }}" style="padding-right: 104px;">
                                ...
                                <div class="tab-pane" id="advanceTab" role="tabpanel">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="params" value="{{ old('params', display_json_value($carrier->params)) }}">
                                    </div>
                                </div>
